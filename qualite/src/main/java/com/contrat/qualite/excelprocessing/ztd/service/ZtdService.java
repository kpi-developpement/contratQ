...
import com.contrat.qualite.excelprocessing.ztd.dto.ZtdGroupDto;
...
public class ZtdService {
    ...
    public ZtdResultDto processZtdExcel(MultipartFile file, int month, int year) {
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8));
            ...
            if (targetCol == null) throw new RuntimeException("Mala9inach l'colonne 'Taux de report TH' f l'fichier ZTD.");
        ...
        kpiArchiveRepository.deleteByMoisAndAnneeAndProcessus(month, year, "ZTD");

        Map<String, ZtdGroupDto> finalDetails = new HashMap<>();
        ...
        for (Map.Entry<String, long[]> entry : statsMap.entrySet()) {
            ...
            finalDetails.put(dept, new ZtdGroupDto(num, denum, resultat, 0.0, 0.0));

            archivesToSave.add(KpiArchive.builder()
                    .mois(month).annee(year).processus("ZTD").departement(dept)
                    .num(num).denum(denum).resultat(resultat).partDeMarche(0.0).bonus(0.0).build());
        }

        kpiArchiveRepository.saveAll(archivesToSave);
        return ZtdResultDto.builder().details(finalDetails).build();
    }
}