...
import com.contrat.qualite.excelprocessing.securisation.dto.SecurisationGroupDto;
...
public class SecurisationService {
    ...
    public SecurisationResultDto processSecurisationExcel(MultipartFile file, int month, int year) {
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8));
            ...
            for (String header : headerMap.keySet()) {
                ...
                if (cleanHeader.contains("flag secu interv")) targetCol = header;
        ...
        kpiArchiveRepository.deleteByMoisAndAnneeAndProcessus(month, year, "SECURISATION");

        Map<String, SecurisationGroupDto> finalDetails = new HashMap<>();
        ...
        for (Map.Entry<String, long[]> entry : statsMap.entrySet()) {
            ...
            finalDetails.put(dept, new SecurisationGroupDto(num, denum, resultat, 0.0));

            archivesToSave.add(KpiArchive.builder()
                    .mois(month).annee(year).processus("SECURISATION").departement(dept)
                    .num(num).denum(denum).resultat(resultat).partDeMarche(0.0).bonus(0.0).build());
        }

        kpiArchiveRepository.saveAll(archivesToSave);
        return SecurisationResultDto.builder().details(finalDetails).build();
    }
}