...
import com.contrat.qualite.excelprocessing.zmdamii.dto.ZmdAmiiGroupDto;
...
public class ZmdAmiiService {
    ...
    public ZmdAmiiResultDto processZmdAmiiExcel(MultipartFile file, int month, int year) {
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8));
            ...
            if (targetCol == null) throw new RuntimeException("Mala9inach l'colonne 'Taux de report TH' f l'fichier ZMD AMII.");
        ...
        kpiArchiveRepository.deleteByMoisAndAnneeAndProcessus(month, year, "ZMD_AMII");

        Map<String, ZmdAmiiGroupDto> finalDetails = new HashMap<>();
        ...
        for (Map.Entry<String, long[]> entry : statsMap.entrySet()) {
            ...
            finalDetails.put(dept, new ZmdAmiiGroupDto(num, denum, resultat, 0.0, 0.0));

            archivesToSave.add(KpiArchive.builder()
                    .mois(month).annee(year).processus("ZMD_AMII").departement(dept)
                    .num(num).denum(denum).resultat(resultat).partDeMarche(0.0).bonus(0.0).build());
        }

        kpiArchiveRepository.saveAll(archivesToSave);
        return ZmdAmiiResultDto.builder().details(finalDetails).build();
    }
}